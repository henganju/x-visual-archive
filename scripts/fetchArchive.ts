...
async function run(){
 ...
 console.log(`Complete: ${state.seen.length} posts inspected; ${state.records.length} photo posts; ${state.records.reduce((n,p)=>n+p.photos.length,0)} photos.\nOldest archived post: ${state.records.at(-1)?.created_at||'none'}\nNewest archived post: ${state.records[0]?.created_at||'none'}\nNo images were downloaded. Rebuild and redeploy after reviewing data.`);
